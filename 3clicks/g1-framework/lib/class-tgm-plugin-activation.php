...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        protected function do_plugin_install() {
            ...
            $is_install_action = isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'];
            $is_update_action = isset( $_GET['tgmpa-update'] ) && 'update-plugin' == $_GET['tgmpa-update'];

            if ( isset( $_GET['plugin'] ) && ( $is_install_action || $is_update_action ) ) {
                ...
                $install_type       = isset ( $_GET['tgmpa-update'] ) ? $_GET['tgmpa-update'] : '';
        ...
        public function notices() {
            ...
            $update_link         = false;   // Set to false, change to true in loop if conditions exist, used for action link 'update'.
            $update_link_count   = 0;       // Used to determine plurality of update action link text.
                    ...
                    if ( isset( $plugin['version'] ) ) {
                        if ( isset( $installed_plugins[$plugin['file_path']]['Version'] ) ) {
                            ...
                            if ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) ) {
                                $update_link = true; // We need to display the 'update' action link
                                $update_link_count++; // Increment the update link count
            ...
            if ( $update_available && isset( $_GET['tgmpa-dismiss'] ) ) {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) || $update_available ) {
                ...
                if ( ! empty( $message ) ) {
                    ...
                    $show_update_link = $update_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'tgmpa' ) . '</a>'  : '';
