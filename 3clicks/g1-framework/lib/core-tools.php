...
{
    $res = preg_replace_callback('!s:(\d+):"(.*?)";!s', 'mb_unserialize_callback', $str );
    return unserialize($res);
}

function mb_unserialize_callback( $m ) {
    ...
    <?php if ( $args['use_microdata'] ) : ?>
		...
		<?php if ( $image ) : ?>
		    <span class="g1-microdata" itemprop="image" itemscope itemtype="http://schema.org/ImageObject">
				...
				<meta itemprop="width" content="<?php echo absint( $image[1] ); ?>" />
				<meta itemprop="height" content="<?php echo absint( $image[2] ); ?>" />
			</span>
		<?php endif; ?>
	<?php endif; ?>
...
if ( ! function_exists( 'g1_render_entry_author' ) ) :
     ...
     */
    function g1_render_entry_author( $args = array() ) {
	    $args = wp_parse_args( $args, array(
			'use_microdata' => false,
		) );
        ?>
        <?php if ( $args['use_microdata'] ) : ?>
			<span class="entry-author" itemscope="" itemprop="author" itemtype="http://schema.org/Person"><?php esc_html_e('by', 'g1_theme'); ?>
		<?php else : ?>
			<span class="entry-author"><?php esc_html_e('by', 'g1_theme'); ?>
		<?php endif; ?>
			...
			printf(
				...
				esc_attr( sprintf( __( 'Posts by %s', 'g1_theme' ), get_the_author() ) )
			);
			?>
				<?php if ( $args['use_microdata'] ) : ?>
...
* @param $args
 */
function g1_render_missing_metadata( $args = array() ) {
	?>
	...
	<span itemprop="publisher" itemscope itemtype="http://schema.org/Organization">
		...
		<span itemprop="logo" itemscope itemtype="http://schema.org/ImageObject">
			<meta itemprop="url" content="<?php echo esc_url( g1_get_microdata_organization_logo_url() ); ?>" />
		</span>
	</span>
	<?php
}
 ...
 */
function g1_get_microdata_organization_logo_url() {
	$url = g1_get_theme_option( 'branding', 'logo_hdpi' );
